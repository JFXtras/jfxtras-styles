...
/**
 * Fluent Button class constructor,
 * i not think what it you needed :).
 */
/*
 todo: add handling new buttons or any node in any parent,
  if new element button - add animation and addPressHandler
  p.s IT FOR ANY NODE for what created custom classes (e.g ToggleButton).
*/
public class FluentButton {
    ...
    public void init(Parent parent) {
        ...
        for (Button button : getButtons(parent)) {
            shrinkAnimationHelperInstance.init(button);
            addPressedHandler(button);
        }
    }
    ...
    private ArrayList<Button> getButtons(Parent root) {
    ...
    private void addPressedHandler(Button button) {
        button.addEventFilter(MouseEvent.MOUSE_PRESSED, (event -> {
            if (event.isSecondaryButtonDown()) {
                if (!isRightClickIsEnabled() && button.getContextMenu() == null) {
                    event.consume();
                }
            }
        }));
