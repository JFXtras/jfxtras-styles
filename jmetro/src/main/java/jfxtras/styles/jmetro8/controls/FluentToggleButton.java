...
/**
 * Fluent Toggle Button class constructor,
...
public class FluentToggleButton {
    ...
    public void init(Parent parent) {
        ...
        for (ToggleButton toggleButton : getToggleButtons(parent)) {
            shrinkAnimationHelperInstance.init(toggleButton);
            addPressedHandler(toggleButton);
        }
    }
    ...
    private ArrayList<ToggleButton> getToggleButtons(Parent root) {
    ...
    private void addPressedHandler(ToggleButton toggleButton) {
        toggleButton.addEventFilter(MouseEvent.MOUSE_PRESSED, (event -> {
            if (event.isSecondaryButtonDown()) {
                if (!rightClickIsEnabled && toggleButton.getContextMenu() == null) {
                    event.consume();
                }
            }
        }));
